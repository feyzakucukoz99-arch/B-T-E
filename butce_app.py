...
def normalize_all(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    if "PersonRef" in df.columns:
        df["PersonRef"] = (
            ...
            .map(lambda s: s if (s.isdigit() and len(s) >= 4) else None)
        )
        df["PersonRef"] = pd.to_numeric(df["PersonRef"], errors="coerce")
    ...
    need("BÜTÇE DIŞI TALEPLER İLE", ["butce disi","budget extra","ekstra","toplam butce disi"], 0.0)
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ...
        df["SİSTEM DIŞI KALAN TL"]   = bdt - (cur*1.415)
        df["SİSTEM DIŞI İLE KULLANILAN ORAN"] = np.where(cur>0, bdt/(cur*1.415), 0.0)
        df["SİSTEM DIŞI İLE KALAN ORAN"]      = np.where(bdt>0, (cur*1.415)/bdt - 1.0, 0.0)

    try:
        st.session_state._personref_source = pr_src
        st.session_state._personref_nonnull = int(df["PersonRef"].notna().sum())
        st.session_state._personref_null = int(df["PersonRef"].isna().sum())
    except Exception:
        pass
...
with st.sidebar:
    ...
    df_sel = df_filtered[df_filtered["PersonRef"].notna()].copy()
    if len(df_sel)==0:
        st.info("Bu yönetici için kişi bulunamadı.")
        selected_ref = None
    else:
        df_sel["REF_INT"] = df_sel["PersonRef"].astype(float).astype(int)
        options = [{"ref": int(r.REF_INT), "name": str(r.FULLNAME)} for _,r in df_sel[["REF_INT","FULLNAME"]].iterrows()]
        def _fmt(o): return f"{o['ref']} — {o['name']}"
        current = None
        if st.session_state.selected_ref is not None:
            for o in options:
                if o["ref"] == int(st.session_state.selected_ref):
                    current = o; break
        sel_obj = st.selectbox("Kişi", options, index= options.index(current) if current else 0, format_func=_fmt)
        st.session_state.selected_ref = int(sel_obj["ref"])
        selected_ref = st.session_state.selected_ref
        st.success(f"Seçili PersonRef: {selected_ref}")
...
sistem_disi_kalan_tl_sum = bdt_sum - (cur_sum*1.415)
sistem_disi_kullanilan_oran = (bdt_sum/(cur_sum*1.415)) if cur_sum>0 else 0.0
sistem_disi_kalan_oran     = ((cur_sum*1.415)/bdt_sum - 1.0) if bdt_sum>0 else 0.0
...
c5.metric("SİSTEM DIŞI İLE KALAN ORAN", pct(sistem_disi_kalan_oran))

st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)

# ================== TABLO (salt görüntü) ==================
view_cols = [
    "PersonRef","FULLNAME","DEPARTMAN",
    ...
    "SİSTEM DIŞI İLE KULLANILAN ORAN","SİSTEM DIŞI İLE KALAN ORAN"
]
df_show = df_filtered[view_cols].copy()
for oc in ["KULLANILAN BÜTÇE ORANI","SİSTEM DIŞI İLE KULLANILAN ORAN","SİSTEM DIŞI İLE KALAN ORAN"]:
...
def islem_yap(person_ref:int, tutar:float, islem_tipi:str, announce=True, do_rerun=True):
    ...
    # 4) Aynı PersonRef'i yeniden bul (yoksa i/ilk satır)
    try:
        mask = pd.to_numeric(dff["PersonRef"], errors="coerce") == float(person_ref)
        if mask.any():
            j = int(dff.index[mask][0])
        else:
            j = int(i) if i in dff.index else int(dff.index[0])
    except Exception:
        j = int(i) if i in dff.index else int(dff.index[0])
...
def handle_command(text: str, ui_amount: float, ui_islem: str, ui_selected_ref: int|None, auto_apply: bool = True):
    ...
    if op and amt is not None and pref is not None:
        if auto_apply or trigger:
            islem_yap(int(pref), float(amt), op); return
        else:
...
with cA:
    if st.button("İşlem Yap", use_container_width=True):
        ...
        if isinstance(man, str) and man.strip():
            try: pref=int(float(man.strip().replace(",", ".")))
            except: pref=None
        if pref is None and st.session_state.get("selected_ref") is not None:
            pref=int(st.session_state.selected_ref)
        if pref is None and last_text:
            pnum, pdig = extract_personref(last_text)
            if pnum is not None: pref=int(pnum); pref_digits=pdig
        if pref is None and last_text:
        ...
        if pref is None:
            ...
            islem_yap(int(pref), float(amt), op, do_rerun=True)
...
# ================== 📊 Seçili Yönetici Oran Grafiği (etiketler sarılır) ==================
st.markdown("## 📊 Seçili Yönetici Oran Grafiği")
oran_full = ["KULLANILAN BÜTÇE ORANI", "SİSTEM DIŞI İLE KULLANILAN ORAN", "SİSTEM DIŞI İLE KALAN ORAN"]
oran_vals = [kullanilan_oran*100.0, sistem_disi_kullanilan_oran*100.0, sistem_disi_kalan_oran*100.0]
...
oran_df = pd.DataFrame({
    "Yönetici": [selected_manager if opts and selected_manager!="(yok)" else "Tümü"]*3,
    "Oran": oran_full,
    "Değer (%)": oran_vals
})
buf_xlsx = io.BytesIO()
